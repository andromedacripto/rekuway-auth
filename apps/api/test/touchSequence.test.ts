...
describe("Touch sequence (3-Touch UX layer)", () => {
  afterAll(async () => {
    ...
    await prisma.organization.deleteMany({ where: { slug: { startsWith: "touchtest-org-" } } });
    await prisma.$disconnect();
  });

  it("stores only a bcrypt hash, never the plaintext sequence", async () => {
    const org = await createTestOrganization(prisma, "touchtest-org");
    const user = await prisma.user.create({
      data: { organizationId: org.id, email: "touchtest+1@example.com" },
    });
  ...
  it("verifies a correct sequence and rejects an incorrect one", async () => {
    const org = await createTestOrganization(prisma, "touchtest-org");
    const user = await prisma.user.create({
      data: { organizationId: org.id, email: "touchtest+2@example.com" },
    });
    ...
    const wrongSymbols = await checkTouchSequence(prisma, user.id, [
      "circle",
      "square",
      "triangle",
    ]);
  ...
  it("returns false for a user with no enrolled sequence", async () => {
    const org = await createTestOrganization(prisma, "touchtest-org");
    const user = await prisma.user.create({
      data: { organizationId: org.id, email: "touchtest+3@example.com" },
    });
